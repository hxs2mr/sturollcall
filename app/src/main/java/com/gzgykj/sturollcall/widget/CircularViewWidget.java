 ...
 * Description on : 自定义圆形进度条
 */
public class CircularViewWidget  extends View {

    private  int height;
    private  int width;
    private Paint mPaint;
    private int strokewith = 5;//线条宽度
    private RectF rectF;
    ...
    public CircularViewWidget(Context context) {
        this(context, null);
    }
    public CircularViewWidget(Context context, AttributeSet attrs)
    {
        this(context, attrs, 0);
    }

    public CircularViewWidget(Context context, AttributeSet attrs ,int defsty)
    {   super(context,attrs,defsty);
        ...
        strokewith = array.getInteger(R.styleable.CircleProgressBar_stroke_width,strokewith);
    ...
    private void initPain() {
        ...
        mPaint.setStrokeWidth(strokewith);         //设置画笔宽度为10px
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        {
            rectF = new RectF(strokewith, (height / 2 - width / 2) + strokewith, width - strokewith, (height / 2 + width / 2) - strokewith);
        } else   if (width > height)//宽大于高的情况
        {
            rectF = new RectF((width / 2 - height / 2) + strokewith, strokewith, (width / 2 + height / 2) - strokewith, height - strokewith);
        } else//宽等于高的情况
        {
            rectF = new RectF(strokewith, strokewith, width - strokewith, height - strokewith);
        }
